...
import   com.danish.dxb.digitify.currency.conversion.BR

abstract class DataBindingRecyclerViewAdapter(viewModels: MutableList<ItemInterface>) :
    RecyclerView.Adapter<DataBindingRecyclerViewHolder<*>>() {

    protected var mViewModels: MutableList<ItemInterface> = ArrayList()
    ...
    @SuppressLint("NotifyDataSetChanged")
    fun refreshList(viewModels: List<ItemInterface>?) {
        mViewModels.clear()
